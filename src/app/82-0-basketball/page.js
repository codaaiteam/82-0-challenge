...
export async function generateMetadata() {
  return rootMetadata('basketballMode', '/82-0-basketball');
}

export default async function BasketballMode({ params }) {
  const locale = params?.lang || 'en';
  const t = locale === 'en' ? en : (await getTranslation(locale)) || en;
  const page = { ...en.pages.basketballMode, ...(t.pages.basketballMode || {}) };
