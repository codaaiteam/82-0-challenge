...
export async function generateMetadata() {
  return rootMetadata('thirtyEight', '/38-0');
}

export default async function ThirtyEight({ params }) {
  const locale = params?.lang || 'en';
  const t = locale === 'en' ? en : (await getTranslation(locale)) || en;
  const page = t.pages.thirtyEight || en.pages.thirtyEight;
  ...
  return (
    <>
      ...
      <main className={styles.mainContent}>
        ...
        {/* Cross-link to the basketball original */}
