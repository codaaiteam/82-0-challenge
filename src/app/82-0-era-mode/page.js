...
export async function generateMetadata() {
  return rootMetadata('eraMode', '/82-0-era-mode');
}

export default async function EraMode({ params }) {
  const locale = params?.lang || 'en';
  const t = locale === 'en' ? en : (await getTranslation(locale)) || en;
  const page = { ...en.pages.eraMode, ...(t.pages.eraMode || {}) };
