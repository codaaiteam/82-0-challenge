...
export async function generateMetadata() {
  return rootMetadata('seventeenZero', '/17-0');
}

export default async function SeventeenZero({ params }) {
  const locale = params?.lang || 'en';
  const t = locale === 'en' ? en : (await getTranslation(locale)) || en;
  const page = t.pages.seventeenZero || en.pages.seventeenZero;
  ...
  return (
    <>
      ...
      <main className={styles.mainContent}>
        ...
        {/* Playable 17-game NFL regular-season variant */}
        <section id="game" className={styles.gameSection}>
          <GameWithSidebarAds>
            <NflGame t={t} games={17} path="/17-0" brand="17-0 CHALLENGE" nflKey="nfl17" />
          </GameWithSidebarAds>
        </section>
        ...
        <section className={styles.howtoSection}>
          <div className={styles.container}>
            <div className={styles.centerCta}>
              <Link href={`${prefix}/20-0#game`} className={styles.heroCta}>
                {page.playCta}
              </Link>
            </div>
          </div>
