...
export async function generateMetadata() {
  return rootMetadata('hardMode', '/82-0-hard-mode');
}

export default async function HardMode({ params }) {
  const locale = params?.lang || 'en';
  const t = locale === 'en' ? en : (await getTranslation(locale)) || en;
  const page = { ...en.pages.hardMode, ...(t.pages.hardMode || {}) };
