...
export async function generateMetadata() {
  return rootMetadata('vsImmaculateGrid', '/82-0-vs-immaculate-grid');
}

export default async function VsImmaculateGrid({ params }) {
  const locale = params?.lang || 'en';
  const t = locale === 'en' ? en : (await getTranslation(locale)) || en;
  const page = { ...en.pages.vsImmaculateGrid, ...(t.pages.vsImmaculateGrid || {}) };
