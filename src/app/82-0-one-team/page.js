...
export async function generateMetadata() {
  return rootMetadata('oneTeam', '/82-0-one-team');
}

export default async function OneTeam({ params }) {
  const locale = params?.lang || 'en';
  const t = locale === 'en' ? en : (await getTranslation(locale)) || en;
  const page = { ...en.pages.oneTeam, ...(t.pages.oneTeam || {}) };
    ...
    <>
      ...
      <main className={styles.mainContent}>
        ...
        <section id="game" className={styles.gameSection}>
          <GameWithSidebarAds>
            <VariantGame t={t} id="oneFranchise" />
          </GameWithSidebarAds>
        </section>
